...
## Overview
This is a snip application that demonstrates how to link with the OTA FW Upgrade library.  The application responsibility is to publish the OTA FW upgrade service in the GATT database and to pass stack callbacks to the library. See the ota\_fw\_upgrade.c file for the  description of the OTA protocol. (typically in mtb\_shared/wiced\_btsdk/libraries/wiced-lib-ota/btsdk-ota/COMPONENT\_fw\_upgrade\_lib). This version of the OTA firmware upgrade relies on the Bluetooth&#174; standard security. The application also can use ECC cryptography to validate the image. It is expected that full implementation will use application level verification that downloaded firmware is for this Product ID and that the image has not been tampered with.
